...
def gui(arg):
    ...
    system.dataWriter._setBufferSize(system.dataWriter,system.dataWriter.bufferSize,1000000)
    system.dataWriter._setMaxFileSize(system.dataWriter,system.dataWriter.maxFileSize,100000000)
     
    if CHECKSPEED:
        for pl_size in [255]:
            today=datetime.date.today()
            today1=today.strftime('%m%d%Y')
            system.dataWriter.dataFile.set("/u1/atlas-chess2-Asic-tests/data/data_h/pre-ampdata-02/Streamreadout_test_"+today1+"_payloadsize_"+str(hex(pl_size))+"_6000Hz.dat")
            system.dataWriter._setOpen(system.dataWriter,system.dataWriter.open,True,1)
            system.feb.sysReg.pktWordSize.set(pl_size)
            print("start!")
            system.feb.sysReg.timingMode.set(0x0)
            while True:
                if (system.dataWriter._getFrameCount(system.dataWriter,system.dataWriter._getFrameCount)>=2000):
